...
describe("DimoriMain.sol", () => {
  ...
  const testRentalTheme = "Urban";
  const testRentalAddress = "NEW YORK";
  ...
  beforeEach(async () => {
    ...
    // Deploy DimoriMain contract
    contractFactory = await ethers.getContractFactory("DimoriMain");
  ...
  describe("Core Functions", () => {
    it("user should be able to add new rental", async () => {
      ...
      expect(userRental[4]).to.equal(testRentalTheme);
      expect(userRental[5]).to.equal(testRentalAddress);
      expect(userRental[6]).to.equal(testRentalLatitude);
      expect(userRental[7]).to.equal(testRentalLongitude);
      expect(userRental[8]).to.equal(testRentaldescription);
      expect(userRental[9]).to.equal(testRentalImageURL);
      expect(userRental[10]).to.equal(testRentalGuestNumber);
      expect(userRental[11]).to.equal(testRentalPricePerDay);
    });
    ...
    it("it should fail by insuffisant amount", async () => {
      ...
      const totalBookingPrice =
        ((numberOfDays - 1) * Number(rentalInfo[11])) / 10 ** 18;
    ...
    it("it should fail by already booked for given dates", async () => {
      ...
      const totalBookingPriceUSD_2 =
        (numberOfDays_2 * Number(rentalInfo[11])) / 10 ** 18;
